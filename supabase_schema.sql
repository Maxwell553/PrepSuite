...
create unique index if not exists players_fide_uscf_unique_idx on public.players (fide_id, uscf_id);

-- 2. Scouting Reports Table
-- Stores the expensive Gemini analysis results.
create table public.scouting_reports (
  id uuid primary key default uuid_generate_v4(),
  user_id uuid references auth.users(id), -- Linked to the authenticated user
...
create index reports_user_idx on public.scouting_reports (user_id);
...
create policy "Users can only see their own reports" on public.scouting_reports for select using (auth.uid() = user_id);
create policy "Users can only insert their own reports" on public.scouting_reports for insert with check (auth.uid() = user_id);
