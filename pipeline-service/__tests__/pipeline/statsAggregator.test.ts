...
describe('generateStats', () => {
  ...
  it('merges "Sicilian" and "Sicilian Defense" into one opening', () => {
    const sicilianShort = makeGamesForOpening(8, {
      side: 'white',
      target: 'player1',
      openingName: 'Sicilian',
      result: '1-0',
    });
    const sicilianFull = makeGamesForOpening(7, {
      side: 'white',
      target: 'player1',
      openingName: 'Sicilian Defense',
      result: '0-1',
    ...
    const games = [...sicilianShort, ...sicilianFull]; // 15 total under "Sicilian Defense"
    const result = generateStats(games, 'player1', 'white');

    expect(result.length).toBe(1);
    expect(result[0].name).toBe('Sicilian Defense');
    expect(result[0].totalGames).toBe(15);
  });
