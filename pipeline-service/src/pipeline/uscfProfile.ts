...
function parseThinPage(html: string, uscfId: string): UscfProfile | null {
  ...
  let name = getValue('memname');
  if (!name) return null;

  // Extract title from name if present: "John Doe (NM)" or ", IM"
  const VALID_USCF_TITLES = ['GM', 'IM', 'FM', 'CM', 'NM', 'WGM', 'WIM', 'WFM', 'WCM', 'WNM'];
  let title = '';
  const titleInNameMatch = name.match(/\(([A-Z]{2,4})\)|,\s*([A-Z]{2,4})\s*$/i);
  if (titleInNameMatch) {
    const t = (titleInNameMatch[1] || titleInNameMatch[2] || '').toUpperCase();
    if (VALID_USCF_TITLES.includes(t)) {
      title = t;
      name = name.replace(/\s*\([A-Z]{2,4}\)|,\s*[A-Z]{2,4}\s*$/gi, '').trim();
    }
  }
  ...
  return { id: uscfId, name, rating, state, fideId, title: title || undefined };
}
...
async function fetchFromMSAScraping(uscfId: string): Promise<UscfProfile | null> {
  try {
    ...
    // Extract title from name: "John Doe (NM)", "Jane Smith, IM", or "US Chess Titles Earned" section
    const VALID_USCF_TITLES = ['GM', 'IM', 'FM', 'CM', 'NM', 'WGM', 'WIM', 'WFM', 'WCM', 'WNM'];
    let title = '';
    const titleInNameMatch = rawName.match(/\(([A-Z]{2,4})\)|,\s*([A-Z]{2,4})\s*$/i);
    if (titleInNameMatch) {
      const t = (titleInNameMatch[1] || titleInNameMatch[2] || '').toUpperCase();
      if (VALID_USCF_TITLES.includes(t)) {
        title = t;
        rawName = rawName.replace(/\s*\([A-Z]{2,4}\)|,\s*[A-Z]{2,4}\s*$/gi, '').trim();
      }
    }
    if (!title) {
      const titlesSection = html.match(/US Chess Titles Earned[\s\S]*?<[^>]+>([^<]+)</i);
      if (titlesSection) {
        const t = titlesSection[1].trim().toUpperCase();
        if (VALID_USCF_TITLES.includes(t)) title = t;
      }
    }
