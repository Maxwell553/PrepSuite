...
export const analyzeRequestSchema = z.object({
  ...
  lichessUsername: z
    ...
    .optional()
    .or(z.literal('')),
  gameLimit: z.number().int().min(1).max(2000).default(1000).optional(),
  onlineLimit: z.number().int().min(0).max(2000).optional(),
  otbLimit: z.number().int().min(0).max(2000).optional(),
}).refine(
  (data) => {
