...
const SearchScreen: React.FC<SearchScreenProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setScanningStatus('Step 1: Resolving player identity (pipeline)...');
        ...
        session.access_token,
        {
          onPhase: (phase, status, durationMs, extra) => {
            if (phase === 'identity' && status === 'started') {
              ...
              setScanningStatus(`Step 4 complete: ${extra?.gamesAnalyzed ?? 0} games analyzed`);
          ...
          onProgress: (phase, current, total) => {
            if (phase === 'games') {
              ...
              setScanningStatus(`Step 4: Stockfish analyzing ${formattedCurrent}/${formattedTotal} games...`);
            }
          },
          onError: (error) => {
